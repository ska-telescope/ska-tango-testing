...
- a way to stop the evaluation of some assertions early and trigger
...
of them. At the moment, the main assertions you can use are:
...
- to give a look to all the support functions and classes we provide
...
- to inform yourself well about the internal mechanisms of the
  ...
  reading the class documentation and the following modules:
...
recognise the custom assertions, but they are there.

**ANOTHER NOTE**: To make assertions about the events order
- i.e., assertions which include a verification with the shape
...
compare events (if it comes from a centralised server and not from the
device itself, because it is important to remember that in distributed
systems the devices' clocks may not be perfectly synchronised).
