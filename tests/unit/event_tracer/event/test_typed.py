...
class TestTypedEvents:
    ...
    def test_event_enum_mapper_create_correct_typed_event() -> None:
        ...
        assert_that(ret_typed_event).is_type_of(TypedEvent)
    ...
    def test_event_enum_mapper_returns_non_typed_event_too() -> None:
        ...
        assert_that(typed_event).is_type_of(ReceivedEvent)
        assert_that(typed_event.device_name).is_equal_to("test/device/1")
    ...
    @staticmethod
    def test_event_enum_mapper_doesnt_type_events_with_none_value() -> None:
        """The EventEnumMapper doesn't type events with None value."""
        event_data = create_eventdata_mock("test/device/1", "state", None)
        ...
        assert_that(typed_event.device_name).is_equal_to("test/device/1")
        assert_that(typed_event.attribute_name).is_equal_to("state")
        assert_that(typed_event.attribute_value).is_none()
